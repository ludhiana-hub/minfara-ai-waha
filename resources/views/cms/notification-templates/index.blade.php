...
@section('title', 'Template Notifikasi WA')
@section('breadcrumb')
    <li class="breadcrumb-item active">Template Notifikasi WA</li>
@endsection

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0 fw-bold">Template Notifikasi WA</h4>
    <a href="{{ route('cms.notification-templates.create') }}" class="btn btn-sm btn-primary">
    ...
    <div class="card-body py-2">
        <form method="GET" class="d-flex flex-wrap gap-2 align-items-center">
            ...
            <input type="text" name="search" class="form-control form-control-sm" style="max-width:220px"
                placeholder="Nama / Trigger key..." value="{{ request('search') }}">
            <button class="btn btn-sm btn-primary"><i class="bi bi-search"></i> Filter</button>
            @if(request()->hasAny(['filter','search']))
                <a href="{{ route('cms.notification-templates.index') }}" class="btn btn-sm btn-light">Reset</a>
            @endif
        </form>
    </div>
</div>
            ...
            <table class="table table-hover mb-0 small">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><code class="text-primary">{{ $t->trigger_key }}</code></td>
                        <td>
                        ...
                        <td class="d-flex gap-1">
                            <a href="{{ route('cms.notification-templates.edit', $t) }}" class="btn btn-sm btn-outline-primary">
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="text-center text-muted py-4">Belum ada template notifikasi</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
...
document.querySelectorAll('.btn-delete').forEach(btn => {
    btn.addEventListener('click', async function() {
        ...
        const res = await fetch(`{{ url('cms-minfara/notification-templates') }}/${id}`, {
            method: 'DELETE',
